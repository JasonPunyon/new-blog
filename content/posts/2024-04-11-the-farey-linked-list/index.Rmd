...
### Approximating $\displaystyle\frac{1}{\phi}$

You can use the Farey Linked list to find rational approximations to irrational numbers like $\phi$
...
pi = graphit(makeList(), "pi");
var left = piList;
var right = piList.next;

var pin = 3141592653589793n
var phi = BigInt(Math.floor(((1 + Math.sqrt(5)) / 2) * 1000000000000000));

console.log(phi);

count = 50;
piTimeout = setInterval(function() {
    newEl = insert(left);

    if (newEl.denominator > Number(phi * newEl.numerator)/1000000000000000) {
        left = newEl;
    } else {
        right = newEl;
    }

    le_element = pi.getElementById(id(newEl));
