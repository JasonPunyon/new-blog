...
let graphit = function(element, destination, label='data(id)', fitpadding=30) {
    ...
    let doLayout = function() {
        cy.nodes().sort(function(n1, n2) {
            ...
            return (l - r) < 0n ? -1 : 1;
        }).layout({ name: "grid", rows: 1, avoidOverlapPadding: 200 }).run();
        cy.fit(focus, fitpadding);
    }
    ...
    cy.on('tap', 'edge', function(evt) {
        ...
        var newnode = cy.add({ group: 'nodes', data: { id: id(newel), sort: newel }});
        focus = newnode;
        cy.add({ 
            group: 'edges', 
            data: { 
                id: `${id(evt.target._private.data.leftnode)} -> ${id(newel)}`,
                source: id(evt.target._private.data.leftnode),
                target: id(newel),
                leftnode: evt.target._private.data.leftnode
            }});
        cy.add({ 
            group: 'edges', 
            data: { 
                id: `${id(newel)} -> ${id(newel.next)}`,
                source: id(newel),
...
In my [last post](/blog/2024/04/10/farey-numbers-and-linked-lists) I talked about using Farey numbers as coordinates for linked list elements. Here's what that looks like. Click the link in the list, and you can insert a new node.

<div id="the-list" class="cy"></div>

```{js, echo=FALSE}
graphit(list, "the-list");
```
