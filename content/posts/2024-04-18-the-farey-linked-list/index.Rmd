...
To find rational approximations of an irrational number, repeatedly insert into the list where that number should be. Let's approximate $\displaystyle\pi$. Wait a minute though, the Farey Numbers only run from $\displaystyle0-1,$ and $\displaystyle\pi > 1.$ So what we'll actually do is approximate $\displaystyle\frac{1}{\pi},$ and take the reciprocal of the answer we get. The display will update each time we get a new best approximation.
...
var left = piList;
var right = piList.next;

var pin = BigInt(Math.PI * 10000000000000000); 

makeApproximationHandler(pin, Math.PI, left, right, pigraph, document.querySelector("button.approx-pi"), document.getElementById("pi-approximation"))
```

That might take awhile, let's do something else in the meantime. Let's do $\displaystyle\phi$.
